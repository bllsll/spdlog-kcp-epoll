...
int udp_output(const char *buf, int len, ikcpcb *kcp, void *user){
   ...
   printf("1111111111111111使用udp_output发送数据\n");
	...
	{
        printf("2222222222kcp回调udp_output: %d bytes send, error\n", n);
        return -1;
    }
}
...
/*
ikcp_create：创建一个新的KCP对象。
ikcp_send：把数据加入发送队列，准备发送。
ikcp_update：更新KCP的内部状态，并处理发送队列中的数据。
ikcp_input：接收对方发送的KCP数据包，并交由KCP处理。
ikcp_recv：从KCP接收队列中获取数据，如果有数据则返回给应用层。
*/
...
// 	int recv_objs = 0;
// 	t_delay_obj objs;
// 		objs.body_str = "client send str";
// 	size_t obj_size = sizeof(objs);
...
// 		ret = ikcp_send(send->pkcp, (char *) &objs, obj_size); //把用户数据封装成KCP的数据包格式，并加入发送队列
// 		printf("客戶端發送send %d str = %s\n", i, objs.body_str.c_str());
//         if(ret < 0) {
//             printf("send %d seqno:%u failed, ret:%d, obj_size:%ld str = %s\n", i, objs.seqno, ret, obj_size, objs.body_str.c_str());
//             return;
//         } 
...
// 			continue;
// 		}
// 		ret = ikcp_input(send->pkcp, buf, n);	// 从 linux api recvfrom先扔到kcp引擎 //接收对方发送的KCP数据包，并交由KCP处理。
// 		if(ret < 0)//检测ikcp_input是否提取到真正的数据
// 		{
// 			//printf("ikcp_input ret = %d\n",ret);
// 			continue;			// 没有读取到数据
// 		}	
// 		ret = ikcp_recv(send->pkcp, (char *)&objs, obj_size); //从KCP接收队列中获取数据，如果有数据则返回给应用层。
// 		if(ret < 0)//检测ikcp_recv提取到的数据	
// 		{
// 			printf("ikcp_recv1 ret = %d\n",ret);
// 			continue;
// 		}
// 		//delay_set_recv_time(&objs);
// 		recv_objs++;
// 		printf("recv1 %d seqno:%d, ret:%d\n", recv_objs, objs.seqno, ret);
//         if(ret != obj_size) {
...
// 			continue;			// 没有读取到数据
// 		}	
// 		ret = ikcp_recv(send->pkcp, (char *)&objs, obj_size);		
// 		if(ret < 0)//检测ikcp_recv提取到的数据	
// 		{
...
// 	for(int i = 0; i < DELAY_TEST2_N; i++) {
// 		char objs[] = "client send str";
// 		size_t obj_size = sizeof(objs);
// 		int ret = ikcp_send(send->pkcp, (char *) &objs, obj_size); //把用户数据封装成KCP的数据包格式，并加入发送队列
// 		if(ret < 0) {
//             //printf("send %d seqno:%u failed, ret:%d, obj_size:%ld str = %s\n", i, objs.seqno, ret, obj_size, objs.body_str.c_str());
//             printf("error");
// 			return;
...
// 		ret = ikcp_recv(send->pkcp, recvBuf, UDP_RECV_BUF_SIZE); //从KCP接收队列中获取数据，如果有数据则返回给应用层。
// 		if(ret < 0)//检测ikcp_recv提取到的数据	
// 		{
// 			printf("ikcp_recv1 ret = %d\n",ret);
// 			continue;
...
void delay_test2(kcpObj *send) {
    ...
    for(int i = 0; i < DELAY_TEST2_N; i++) {
		...
		ret = ikcp_recv(send->pkcp, body_str, obj_size); //从KCP接收队列中获取数据，如果有数据则返回给应用层。
		if(ret < 0)//检测ikcp_recv提取到的数据	
		{
			printf("ikcp_recv1 ret = %d\n",ret);
			continue;
		...
		//delay_set_recv_time(&objs);
		recv_objs++;
		printf("recv1 %d, ret:%d\n", recv_objs, ret);
        if(ret != obj_size) {
            printf("recv1 %d failed, size:%d\n", i, ret);
            //delay_print_rtt_time(objs, i);
            return;
        }
    }
