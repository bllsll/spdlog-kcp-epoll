...
// 编译:  g++ -o server server.cpp delay.cpp ikcp.c  -lpthread
...
void loop(kcpObj *send)
{
	...
	while (1)
	{
		...
		ikcp_update(send->pkcp, iclock());
		//处理收消息
		n = recvfrom(send->sockfd, buf, RECV_BUF, MSG_DONTWAIT, (struct sockaddr *)&send->ClientAddr, &len);
		if (n > 0)
		{
			printf("UDP11111 recv[%d]  size= %d\n", recv_count++, n);
			//t_delay_obj *delay = (t_delay_obj *)buf;
			//printf("UDP recv[%d]  size= %d %s\n", recv_count++, n, delay->body_str.c_str());
